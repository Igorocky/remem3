    ...
    public String getSqlText() {
        ...
        if (foreignKey != null) {
            if (foreignKey.getTable() == null) {
                throw new RuntimeException("Foreign key table must not be null.");
            }
            if (StringUtils.isBlank(foreignKey.getTable().getName())) {
                throw new RuntimeException("Foreign key table name must not be blank.");
            }
            if (foreignKey.getOnUpdate() == null) {
                throw new RuntimeException("Foreign key 'on update' must not be null.");
            }
            if (foreignKey.getOnDelete() == null) {
                throw new RuntimeException("Foreign key 'on delete' must not be null.");
            }
